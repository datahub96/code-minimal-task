...
const DailyPlannerWithSupabase: React.FC<DailyPlannerProps> = ({
  onClose = () => {},
  onAddTasks = () => {},
}) => {
      ...
      try {
        ...
        if (supabase && userId) {
          ...
          if (error && error.code !== "PGRST116") {
            // PGRST116 is "no rows returned" which is expected for new dates
            console.error("Supabase error loading planner:", error);
            throw error;
          }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError("Failed to save planner data. Please try again.");
    } finally {
      setLoading(false);
    }
  };
...
export default DailyPlannerWithSupabase;
