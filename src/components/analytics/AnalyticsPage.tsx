  ...
  return (
    <div className="flex flex-col h-full bg-background p-2 md:p-6 overflow-auto">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid grid-cols-3 md:grid-cols-5 mb-4 overflow-x-auto">
          <TabsTrigger
            value="overview"
        ...
        <TabsContent value="trends" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-lg font-semibold flex items-center">
                <Calendar className="mr-2 h-5 w-5" /> Task Completion Timeline
              </CardTitle>
            </CardHeader>
            <CardContent>
                ...
                {filteredTasks
                  ...
                  .map((task) => {
                    ...
                    return (
                      <div
                        key={task.id}
                        className="relative pl-6 pb-8 border-l border-muted last:pb-0"
                      >
                ...
                <ScrollArea className="h-[300px] pr-4">
                  <div className="space-y-4">
                    <div className="p-4 border rounded-lg">
                      <h3 className="font-medium">Most Productive Category</h3>
                      {Object.entries(completionRateByCategory).length > 0 ? (
                        <div className="mt-2">
                          {Object.entries(completionRateByCategory)
                            .filter(([, { total }]) => total >= 2) // At least 2 tasks to be meaningful
                            .sort(
                              ([, a], [, b]) =>
                    ...
                    <div className="p-4 border rounded-lg">
                      <h3 className="font-medium">Most Efficient Category</h3>
                      {Object.entries(timeByCategory).length > 0 ? (
                        <div className="mt-2">
                          {Object.entries(timeByCategory)
                            .filter(([category]) => {
                              const completed =
                                completionRateByCategory[category]?.completed ||
                                0;
                              return completed >= 1; // At least 1 completed task
                            })
                            .sort(([categoryA], [categoryB]) => {
                              const completedA =
                                completionRateByCategory[categoryA]
                                  ?.completed || 0;
                              const completedB =
                                completionRateByCategory[categoryB]
                                  ?.completed || 0;
                              const timePerTaskA =
                                completedA > 0
                                  ? timeByCategory[categoryA] / completedA
                                  : Infinity;
                              const timePerTaskB =
                                completedB > 0
                                  ? timeByCategory[categoryB] / completedB
                                  : Infinity;
                              return timePerTaskA - timePerTaskB;
                            })
                            .slice(0, 1)
                            .map(([category, timeSpent]) => {
                              const completed =
                                ...
                                0;
                              const avgTime =
                                completed > 0 ? timeSpent / completed : 0;
                              return (
                                <div key={category}>
                                  <div className="flex justify-between items-center">
                                    <span className="text-sm">{category}</span>
                                    <span className="text-sm font-medium">
                                      {formatTimeSpent(avgTime)} per task
                                    </span>
                                  </div>
                                  <p className="text-xs text-muted-foreground mt-1">
                            ...
                            .map(([category, timeSpent]) => {
                              ...
                              return (
                                <div key={category}>
                                  ...
                                  <p className="text-xs text-muted-foreground mt-1">
                                    {percentage.toFixed(0)}% of total time spent
                                  </p>
                                  <Progress
                                    value={percentage}
                                    className="h-1 mt-2"
                ...
                <div className="space-y-6">
                  {recentActivity.map((task) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`absolute left-0 top-0 -translate-x-1/2 w-3 h-3 rounded-full ${isCompleted ? "bg-green-500" : "bg-blue-500"}`}
                        ></div>
                        <div>
                          <div className="flex justify-between items-start">
                            <h3 className="font-medium">{task.title}</h3>
                          ...
                          {task.timeSpent && task.timeSpent > 0 && (
                            <div className="mt-2 text-xs inline-flex items-center bg-muted px-2 py-1 rounded">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {((completedTasks / totalTasks) * 100).toFixed(0)}% completion
                  rate
                </p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {Object.keys(tasksByCategory).length}
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  Most active:{" "}
                  {Object.entries(tasksByCategory).sort(
                    ([, a], [, b]) => b - a,
                  )[0]?.[0] || "None"}
                </p>
              </CardContent>
